...
export default class CronClientesFaturasLateFortyDaysService {
  public async execute(): Promise<any> {
    console.log('Inicio busca clientes com faturas vencidas há 40 dias');
    const now = new Date();
    ...
    const responseCriarLista = await apiPlanet.post(
      'criarLista',
      stringify({
        nome: `Clientes_Com_Faturas_Vencidas_40_Dias_${now.toJSON()}`,
        id_campanha: process.env.ID_CAMPANHA_BOLETOS_VENCIDOS_40_DIAS,
        ativa: false,
        formato_mailing:
          process.env.ID_FORMATO_MAILING_BOLETOS_VENCIDOS_40_DIAS,
      }),
      {
        headers: {
    ...
    console.log('Fim busca clientes com faturas vencidas há 40 dias');
    return clientes;
  }
}
